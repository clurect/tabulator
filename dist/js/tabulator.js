...
;(function (global, factory) {
	...
	RowManager.prototype._virtualRenderFill = function (position, forceMove, offset) {
		...
		if (self.displayRowsCount && Tabulator.prototype.helpers.elVisible(self.element)) {
			...
			if (forceMove && this.element.scrollWidth > this.element.offsetWidth) {
			...
			if (dir) {
				...
				if (bottomDiff >= 0) {
					var update = function update() {
						_this6._addBottomRow(bottomDiff);
					};
	...
	Row.prototype.updateData = function (data) {
		...
		return new Promise(function (resolve, reject) {
			...
			for (var attrname in data) {
				var cell = _this7.getCell(attrname);
			...
			if (self.table.options.dataTree !== false && self.table.modExists("dataTree") && typeof data[_this7.table.modules.dataTree.getChildField()] !== "undefined") {
	...
	Row.prototype.delete = function () {
		...
		return new Promise(function (resolve, reject) {
			var index = _this8.table.rowManager.getRowIndex(_this8);
	...
	Clipboard.prototype.buildOutput = function (rows, config, params) {
		...
		if (config.rowGroups) {
			rows.forEach(function (row) {
				output = output.concat(_this27.parseRowGroupData(row, config, params, calcs || {}));
			});
		} else {
	...
	Group.prototype.createValueGroups = function () {
		...
		if (this.groupManager.allowedValues && this.groupManager.allowedValues[level]) {
			this.groupManager.allowedValues[level].forEach(function (value) {
				_this36._createGroup(value, level);
			});
		}
	};
	...
	Page.prototype.generatePageSizeSelectList = function () {
		...
		if (this.pageSizeSelect) {
			...
			while (this.pageSizeSelect.firstChild) {
				...
				_this38.pageSizeSelect.appendChild(itemEl);
			});
	...
	Page.prototype.setPage = function (page) {
		...
		return new Promise(function (resolve, reject) {
			...
			if (page > 0 && page <= _this39.max) {
				...
				console.warn("Pagination Error - Requested page is out of range of 1 - " + _this39.max + ":", page);
				reject();
			}
		});
	};
	...
	Page.prototype.setPageToRow = function (row) {
		...
		return new Promise(function (resolve, reject) {
			...
			var rows = _this40.table.rowManager.getDisplayRows(_this40.displayIndex - 1);
			var index = rows.indexOf(row);

			if (index > -1) {
				var page = Math.ceil((index + 1) / _this40.size);

				_this40.setPage(page).then(function () {
					resolve();
				}).catch(function () {
					reject();
				});
	...
	Page.prototype.nextPage = function () {
		...
		return new Promise(function (resolve, reject) {
			if (_this42.page < _this42.max) {
				...
				if (!_this42.progressiveLoad) {
					console.warn("Pagination Error - Next page would be greater than maximum page of " + _this42.max + ":", _this42.max + 1);
				}
				reject();
			}
		});
	...
	Page.prototype.trigger = function () {
		...
		return new Promise(function (resolve, reject) {
			...
			switch (_this43.mode) {
				case "local":
					left = _this43.table.rowManager.scrollLeft;

					_this43.table.rowManager.refreshActiveData("page");
					_this43.table.rowManager.scrollHorizontal(left);

					_this43.table.options.pageLoaded.call(_this43.table, _this43.getPage());
					resolve();
					break;
				...
				case "progressive_scroll":
					_this43.table.modules.ajax.blockActiveRequest();
					_this43._getRemotePage().then(function () {
						resolve();
					}).catch(function () {
						reject();
					});
				...
				default:
					console.warn("Pagination Error - no such pagination mode:", _this43.mode);
					reject();
			}
		});
	};
	...
	Page.prototype._getRemotePage = function () {
		...
		return new Promise(function (resolve, reject) {
			...
			pageParams[_this44.paginationDataSentNames.page] = self.page;

			//set page size if defined
			if (_this44.size) {
				pageParams[_this44.paginationDataSentNames.size] = _this44.size;
			}

			//set sort data if defined
			if (_this44.table.options.ajaxSorting && _this44.table.modExists("sort")) {
				var sorters = self.table.modules.sort.getSort();
				...
				pageParams[_this44.paginationDataSentNames.sorters] = sorters;
			}

			//set filter data if defined
			if (_this44.table.options.ajaxFiltering && _this44.table.modExists("filter")) {
				var filters = self.table.modules.filter.getFilters(true, true);
				pageParams[_this44.paginationDataSentNames.filters] = filters;
			}

			self.table.modules.ajax.setParams(pageParams);

			self.table.modules.ajax.sendRequest(_this44.progressiveLoad).then(function (data) {
